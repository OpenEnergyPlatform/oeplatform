...
class TableTags(Base):
    ...
    __table_args__ = (
        PrimaryKeyConstraint('tag', 'schema_name', 'table_name', name='table_tags_pkey'),
        {'schema': 'public'},
    )
...
class EditBaseOld(Base):
    ...
    _submitted = Column('_submitted', DateTime, nullable=False, server_default=text("now()"))
    _autocheck = Column('_autocheck', Boolean, nullable=False, server_default=text("false"))
    _humancheck = Column('_humancheck', Boolean, nullable=False, server_default=text("false"))
