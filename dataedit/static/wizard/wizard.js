...
var Wizard = function(config) {
    // TODO: uploadChunkSize to proper size after testing
    var state = {
      schema:"model_draft", 
      apiVersion:"v0", 
      ...
      cursor_id:null, 
      previewRows:null, 
      csvParser:null, csvColumns:null, rowMapper:null, skippedHeader:null, uploadProgressBytes:null, fileSizeBytes:null, uploadedRows:null, fileName:null, cancel:null, };
    var columnParsers = {"parseFloat2":{parse:parseFloat2, label:"Number (Englisch)"}, "parseFloatGerman":{parse:parseFloatGerman, label:"Number (German)"}, "":{parse:function(x) {
      return x;
    }, label:""}};
    ...
    function getWizardUrl(tablename) {
      tablename = tablename || state.table;
      return "/dataedit/upload/" + state.schema + "/" + tablename;
    }
    ...
    function csvUpload() {
      ...
      sendJson("POST", getApiAdvancedUrl("connection/open")).then(function(res) {
        ...
        state.file.parse({config:{encoding:state.encoding, skipEmptyLines:state.skipEmptyLines, preview:state.previewSizeRecords + (state.header ? 1 : 0), delimiter:state.delimiter, newline:state.newline, chunkSize:state.uploadChunkSize, chunk:function(data, parser) {
          if (state.cancel) {
            rollback("cancel");
            return;
          }
