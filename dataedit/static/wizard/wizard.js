var Wizard = function(config) {
    var state = {schema:"model_draft", apiVersion:"v0", previewSizeRecords:10, exampleRows:5, csvColumnPrefix:"Column_", uploadChunkSize:1024 * 10, newline:"\n", skipEmptyLines:"greedy", canAdd:config.canAdd, columns:config.columns, table:config.table, nRows:config.nRows, connection_id:null, cursor_id:null, previewRows:null, csvParser:null, csvColumns:null, rowMapper:null, skippedHeader:null, uploadProgressBytes:null, fileSizeBytes:null, uploadedRows:null, fileName:null, cancel:null, };
    var columnParsers = {"parseFloatCommaDecimal":{parse:parseFloatCommaDecimal, label:"Number with comma as decimal"}, "":{parse:function(x) {
      return x;
    }, label:""}};
                  ...
                  if (/varchar[ ]*\(([0-9]+)\)/.exec(data_type)) {
                    ...
                    if (/char[ ]*\(([0-9]+)\)/.exec(data_type)) {
                      ...
                      if (/bool/.exec(data_type)) {
                        ...
                        if (/text|varchar|char/.exec(data_type)) {
    ...
    function getErrorMsg(x) {
      try {
        x = JSON.parse(x.responseJSON).reason;
      } catch (e) {
        x = x.statusText;
      }
      return x;
    ...
    function sendJson(method, url, data, success, error) {
      ...
      return $.ajax({url:url, headers:{"X-CSRFToken":token}, data_type:"json", cache:false, contentType:"application/json; charset=utf-8", processData:false, data:data, type:method, converters:{"text json":function(data) {
        return data;
    ...
    function changeFileSettings() {
      ...
      if (state.file) {
        state.file.parse({config:{encoding:state.encoding, skipEmptyLines:state.skipEmptyLines, preview:state.previewSizeRecords + (state.header ? 1 : 0), delimiter:state.delimiter, newline:state.newline, complete:function(result, _file) {
          if (result.data.length) {
            removeNewline(result.data);
            getDomItem("csv-text").text(result.data.map(function(row) {
    ...
    function createContext(insertValues) {
      ...
      if (insertValues) {
        var query = {schema:state.schema, table:state.table, fields:state.columns ? state.columns.map(function(e) {
          return e.name;
        }) : undefined, values:insertValues};
    ...
    function csvUpload() {
      ...
      sendJson("POST", getApiAdvancedUrl("connection/open")).then(function(res) {
        ...
        state.file.parse({config:{encoding:state.encoding, skipEmptyLines:state.skipEmptyLines, delimiter:state.delimiter, newline:state.newline, chunkSize:state.uploadChunkSize, chunk:function(data, parser) {
          if (state.cancel) {
            rollback("cancel");
            return;
          }
          ...
          sendJson("POST", getApiAdvancedUrl("connection/commit"), createContext()).then(function() {
            ...
            setStatusUpload("success", false, "Upload ok: " + state.uploadedRows + " rows");
            resetUpload();
          });
        }, error:function(error) {
