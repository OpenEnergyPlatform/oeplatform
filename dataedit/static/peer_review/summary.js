    ...
    fieldsForCategory.forEach((field) => {
      ...
      if (words.length === 1) {
        ...
        if (!groupedFields[prefix])
          groupedFields[prefix] = { indexed: {}, noIndex: [] };

        if (indices.length > 0) {
          const indexKey = indices
            .map((num) => parseInt(num, 10) + 1)
            .join(".");
          if (!groupedFields[prefix].indexed[indexKey])
            groupedFields[prefix].indexed[indexKey] = [];
          groupedFields[prefix].indexed[indexKey].push({
            ...field,
            fieldName: nameWithoutIndices,
          });
        } else {
          groupedFields[prefix].noIndex.push({
            ...field,
            fieldName: nameWithoutIndices,
          });
        }
    ...
    if (Object.keys(groupedFields).length > 0) {
      ...
      for (const prefix in groupedFields) {
        ...
        if (Object.keys(indexed).length > 0) {
          ...
          Object.entries(indexed).forEach(
            ([idx, idxFields], idxAccordionIndex) => {
              ...
              const tabLabel = ["source", "license"].includes(category)
                ? "fields"
                : `${prefix} ${idx}`;
