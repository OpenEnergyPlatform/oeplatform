    ...
    fetch(metaUrl)
      ...
      .then((data) => {
        ...
        if (metaObj && metaObj.resources && metaObj.resources.length > 0) {
          ...
          if (projectName) {
            projectHtml = `<span class="me-3 mb-2 text-muted" data-bs-toggle="tooltip" title="Project Name"><i class="fas fa-project-diagram me-1"></i>Project: <span class="fw-medium text-dark">${projectName}</span></span>`;
          }
          ...
          if (creatorOrg) {
            orgHtml = `<span class="me-3 mb-2 text-muted" data-bs-toggle="tooltip" title="Creator Organization"><i class="fas fa-building me-1"></i>Org: <span class="fw-medium text-dark">${creatorOrg}</span></span>`;
          }

          // License
          if (res.licenses && res.licenses.length > 0) {
            licenseHtml = `<span class="badge bg-light text-dark border me-2 mb-2" data-bs-toggle="tooltip" title="${res.licenses[0].title || "Data License"}"><i class="fas fa-balance-scale me-1"></i>License: ${res.licenses[0].name}</span>`;
          }

          // Spatial
          if (res.spatial && res.spatial.extent && res.spatial.extent.name) {
            spatialHtml = `<span class="me-3 mb-2 text-muted" data-bs-toggle="tooltip" title="Spatial Extent / Location"><i class="fas fa-map-marker-alt me-1"></i>Location: <span class="fw-medium text-dark">${res.spatial.extent.name}</span></span>`;
          }

          // Temporal
          if (res.temporal && res.temporal.referenceDate) {
            temporalHtml = `<span class="me-3 mb-2 text-muted" data-bs-toggle="tooltip" title="Temporal Reference"><i class="far fa-calendar-alt me-1"></i>Ref Date: <span class="fw-medium text-dark">${res.temporal.referenceDate}</span></span>`;
          }

          // Peer Review Badge
          if (res.review && res.review.badge) {
            reviewHtml = `<span class="badge bg-success bg-opacity-10 text-success border border-success me-2 mb-2" data-bs-toggle="tooltip" title="Peer Review Status"><i class="fas fa-award me-1"></i>${res.review.badge} Reviewed</span>`;
          }
        }
