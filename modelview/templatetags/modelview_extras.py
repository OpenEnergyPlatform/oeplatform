...
def develop_year(model, label):
    ...
    if kind == "not estimated":
        ...
        s = "{amount}% {kind} {year}".format(
            amount=model.__dict__[label + "_amount"],
            kind=kind,
            year=model.__dict__[label + "_year"],
        )
    return format_html(
        "<tr><td class='sheetlabel'>{label}</td><td>{s}</td></tr>".format(
            label=model._meta.get_field(label + "_amount").verbose_name, s=s
        )
    )
