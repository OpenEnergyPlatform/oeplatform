...
class Energyscenario(models.Model):
    name_of_the_study = CharField(verbose_name='Name of the study', help_text='What is the name of the study?', max_length=100) 
    author_Institution = CharField(verbose_name='Author, Institution', help_text='Who are the authors of the study and for which institution do they work?', max_length=100) 
    client = CharField(verbose_name='Client', help_text='Who are the customers requesting the study?', max_length=100) 
    ...
    citation_reference = ArrayField(CharField(verbose_name='Citation reference', max_length=100))
    citation_doi = ArrayField(CharField(verbose_name='Citation doi', max_length=100))
    aim = CharField(verbose_name='Aim', help_text='What is the purpose (hypothesis) and research question of the study?', max_length=100) 
    new_aspects = CharField(verbose_name='New aspects', help_text='What is new? (beyond state of research)', max_length=100) 
    spatial_Geographical_coverage = CharField(verbose_name='Spatial / Geographical coverage', help_text='Which geographical region is adressed in the study?', max_length=100) 
    ...
    time_frame_other_text = CharField(max_length=100)
    ...
    tools_models = ForeignKey(to="Energymodel", verbose_name='Tools', help_text='Which model(s) and other tools have been used?') 
    tools_other = CharField(verbose_name='Tools', help_text='Which model(s) and other tools have been used?' , max_length=100) 
    ...
    modeled_energy_sectors_others_text = CharField(max_length=100)
    ...
    economic_behavioral_other_text = CharField(max_length=100) 
    ...
    renewables_others_text = CharField(max_length=100) 
    ...
    economic_focuses_included = CharField(verbose_name='Economic focuses included', help_text='Have there been economic focusses/sectors included?', max_length=100) 
    social_focuses_included = CharField(verbose_name='Social focuses included', help_text='Have there been social focusses/sectors included? ', max_length=100) 
    endogenous_variables = CharField(verbose_name='Endogenous variables', help_text='Which time series and variables are generated inside the model? (Does this question belong into the scenario fact sheet?)', max_length=100) 
    ...
    time_steps_other_text = CharField(max_length=100)
    ...
    exogenous_time_series_used_others_text = CharField(max_length=100) 
    
    technical_data = CharField(verbose_name='Technical data + usage', help_text='What kind of technical data(sets) are included /used? (heat-/powerplants; grid infrastructure;...) What were the data(sets) used for (e.g. model calibration)?', max_length=100) 
    social_data = CharField(verbose_name='Social data', help_text='What kind of social data(sets) are included / were used / considered? (e.g. demographic changes, employment rate; social structure, ...) What were the data(sets) used for (e.g. model calibration)?', max_length=100) 
    economical_data = CharField(verbose_name='Economical data', help_text='What kind of economical data(sets) are included / were used? (e.g. price structures, market settings,...) What were the data(sets) used for (e.g. model calibration)?', max_length=100) 
    ecological_data = CharField(verbose_name='Ecological data', help_text='What kind of ecological data(sets) are included / were used? (e.g. landuse, CO2 emissions,...) What were the data(sets) used for (e.g. model calibration)?', max_length=100) 
    preProcessing = CharField(verbose_name='Pre-Processing', help_text='Have the mentioned values been modified before being used for the modelling exercise or are they used directly? Please, describe what kind of modification have been made? Additionally, you can link to data processing scripts.', max_length=100) 
    name_of_scenario = CharField(verbose_name='Name of the Scenario', help_text='What is the name of the scenario?', max_length=100) 
    name_of_study = URLField(verbose_name='Name of the study', help_text='Which study does the scenario belong to?') 

    energy_saving_amount = SmallIntegerField(verbose_name='Energy saving', help_text='development of energy savings or efficiency') 
    energy_saving_until = BooleanField(verbose_name='[x] % until [20xx]') 
    energy_saving_per = BooleanField(verbose_name='[x] % per [year]') 
    energy_saving_ne = BooleanField(verbose_name='not estimated') 

    potential_energy_savings_amount = SmallIntegerField(verbose_name='Potential energy saving', help_text='How was the potential of energy savings determined?') 
    potential_energy_savings_until = BooleanField(verbose_name='[x] % until [20xx]') 
    potential_energy_savings_per = BooleanField(verbose_name='[x] % per [year]') 
    potential_energy_savings_ne = BooleanField(verbose_name='not estimated') 

    emission_reductions_amount = SmallIntegerField(verbose_name='Potential energy saving', help_text='Development of emissions') 
    emission_reductions_until = BooleanField(verbose_name='[x] % until [20xx]') 
    emission_reductions_per = BooleanField(verbose_name='[x] % per [year]') 
    emission_reductions_ne = BooleanField(verbose_name='not estimated') 

    share_RE_amount = SmallIntegerField(verbose_name='Share RE (power sector)', help_text='Development of renewable energy in the power sector') 
    share_RE_power_until = BooleanField(verbose_name='[x] % until [20xx]') 
    share_RE_power_per = BooleanField(verbose_name='[x] % per [year]') 
    share_RE_power_ne = BooleanField(verbose_name='not estimated') 

    emission_reductions_amount = SmallIntegerField(verbose_name='Share RE (heat sector)', help_text='development of renewable energy in the heat sector') 
    share_RE_heat_until = BooleanField(verbose_name='[x] % until [20xx]') 
    share_RE_heat_per = BooleanField(verbose_name='[x] % per [year]') 
    share_RE_heat_ne = BooleanField(verbose_name='not estimated') 

    emission_reductions_amount = SmallIntegerField(verbose_name='Share RE (mobility sector)', help_text='development of renewable energy in the mobility sector') 
    share_RE_mobility_until = BooleanField(verbose_name='[x] % until [20xx]') 
    share_RE_mobility_per = BooleanField(verbose_name='[x] % per [year]') 
    share_RE_mobility_ne = BooleanField(verbose_name='not estimated') 

    emission_reductions_amount = SmallIntegerField(verbose_name='Share RE (total energy supply)', help_text='development of total renewable energy supply') 
    share_RE_total_until = BooleanField(verbose_name='[x] % until [20xx]') 
    share_RE_total_per = BooleanField(verbose_name='[x] % per [year]') 
    share_RE_total_ne = BooleanField(verbose_name='not estimated') 
    ...
    technological_innovations = CharField(verbose_name='Technological innovations', help_text='Have future technological innovations been regarded?', max_length=100) 
    ...
    potential_wind_other_text = CharField(max_length=100)
    ...
    potential_solar_electric_other_text = CharField(max_length=100)
    ...
    potential_solar_thermal_other_text = CharField(max_length=100)
    ...
    potential_biomass_other_text = CharField(max_length=100)
    ...
    potential_geothermal_othertext = BooleanField(verbose_name='other')
    potential_geothermal_othertext = CharField(max_length=100)
    ...
    potential_hydro_power_other_text = CharField(max_length=100)

    social_developement = CharField(verbose_name='Social developement', help_text='How are changes of social structure considered? (e.g. demographic changes, employment rate, ...)', max_length=100) 
    economic_development = CharField(verbose_name='Economic development', help_text='e.g. price structures, market settings,..', max_length=100) 
    development_of_environmental_aspects = CharField(verbose_name='Development of environmental aspects', help_text='e.g. landuse', max_length=100) 
    ...
    uncertainty_assessment = CharField(verbose_name='Uncertainty assessment', help_text='How are the identified uncertain factors considered in the study?', max_length=100) 
    robustness = CharField(verbose_name='Robustness', help_text='How is the robustness of the results proofed?', max_length=100) 
    comparability_Validation = CharField(verbose_name='Comparability / Validation', help_text='How far do the modelling results fit in compared to similar scientific research?', max_length=100) 
    conclusions = CharField(verbose_name='Conclusions', help_text='What political, social (or in another way) relevant conclusions are drawn from the scenario analysis? ', max_length=100) 
