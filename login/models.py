...
class TablePermission(models.Model):
    ...
    table = models.ForeignKey(
        ...
        # as '<subclass>_items' (all lower case)
    )
    ...
    def get_tables_queryset(
        ...
        return (
            datamodels.Table.objects.filter(
                ...
                userpermission_items__holder=self,
                userpermission_items__level__gte=min_permission_level,
            )
            | datamodels.Table.objects.filter(
                # tables where user isin a group that has GroupPermission
                grouppermission_items__holder__in=groups,
                grouppermission_items__level__gte=min_permission_level,
            )
        ).distinct()
