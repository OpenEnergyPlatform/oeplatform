...
# page or an error document being written to disk as "the validator". It bounds
# what is accepted, not what is read: the body is already in memory when it is
# checked, which is fine at this size.
...
class UnusableOntologyError(ShapeArtifactError):
    """The OEO release cannot yield a label subset.

    One class rather than three, because the caller does the same thing with
    all of them: the release is absent, unparseable, or carries no labels.
    Which one it was is in the message.
    """
...
def fetch_shape_payload(
    url: str,
    *,
    read_url: Optional[Callable[[str], bytes]] = None,
) -> bytes:
    """Fetch the SHACL shape from ``url`` and return it, or refuse it.

    Returns bytes rather than writing, so the caller can obtain both artifacts
    before it replaces either. The body is parsed and checked to be a shape
    graph here, so an error document or a truncated response is refused before
    it can reach the disk at all.
    """
...
def build_label_subset_payload(oeo_full_owl: Path, oeo_version: str = "") -> bytes:
    """Build the ``rdfs:label``-only subset of an OEO release.
    ...
    ``oeo_version`` is written into the file as a comment. Two of the three
    environments fetch the ontology from an unpinned "latest" release, so the
    ...
    try:
        ...
        raise UnusableOntologyError(
            f"{oeo_full_owl} could not be parsed ({error}), so the label "
    ...
    header = (
        "# rdfs:label subset generated by manage.py fetch_oekg_shapes\n"
