...
class TestMarkdown(TestCase):
    ...
    def test_script(self):
        html = md_to_html(
            ["<script>alert('hi');</script>", "text", "<script>alert('hi');</script>"]
        )
        self.assertRegexpMatches(html, "text")

    def test_autocorrect_html_964(self):
        """automatically balance tags in raw html."""
        html = md_to_html(["<div>test</div></div>"])
        self.assertEquals(html, "<div>test</div>")
        html = md_to_html(["<div><div>test</div>"])
        self.assertEquals(html, "<div><div>test</div></div>")
