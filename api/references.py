...
class Entry(Base):
    ...
    entries_id = Column(
        Integer,
        primary_key=True,
        server_default=text("nextval('reference.entries_entries_id_seq'::regclass)"),
    )
...
class Group(Base):
    ...
    groups_id = Column(
        Integer,
        primary_key=True,
        server_default=text("nextval('reference.groups_groups_id_seq'::regclass)"),
    )
    ...
    search_expression = Column(
        String(200), server_default=text("NULL::character varying")
    )
...
class String(Base):
    ...
    strings_id = Column(
        Integer,
        primary_key=True,
        server_default=text("nextval('reference.strings_strings_id_seq'::regclass)"),
    )
    label = Column(String(100), server_default=text("NULL::character varying"))
...
def read_bibtexfile(file_name):
    engine = create_engine("postgres://ckan_perm:H6LgV#C&Mye(arGxQ2B-@localhost/test")
