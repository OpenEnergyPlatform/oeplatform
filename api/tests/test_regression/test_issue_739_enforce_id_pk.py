...
class Test_issue_739_enforce_id_pk(APITestCase):    
    ...
    def create_table(self, table_definition, table=None):
        ...
        table_url = reverse("api_table", kwargs={"schema": self.test_schema, "table": table})
    ...
    def test_check_insert_issue_739(self):
        ...
        res = self.create_table({"columns": [{'name': "not_id", 'data_type': 'int'}]}, table)
        ...
        url = reverse("api_rows_new", kwargs={"schema": self.test_schema, "table": table})
    ...
    def test_id_type_and_pk(self):                
        ...
        res = self.create_table({"columns": [{'name': "id", 'data_type': 'int', "primary_key": True}]}, "test_id_ok")
        self.assertEqual(res.status_code, 201)

        res = self.create_table({"columns": [{'name': "id", 'data_type': 'int'}]}, "test_id_auto_create_pk")
        self.assertEqual(res.status_code, 201)

        res = self.create_table({"columns": [{'name': "id", 'data_type': 'float'}]}, "test_id_wrong_type")
        self.assertEqual(res.status_code, 400)

        res = self.create_table({"columns": [{'name': "id", 'data_type': 'int'}, {'name': "id2", 'data_type': 'int', "primary_key": True}]}, "test_id_wrong_pk2")
        self.assertEqual(res.status_code, 400)
